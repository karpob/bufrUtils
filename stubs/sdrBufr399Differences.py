...
def bufr_decode(input_file):
    ...
    while True:
        ...
        for i,s in enumerate(seconds):
            ...
            dates.append( datetime(year ,month, day, hour, m, int(s), int(1.0e6*(s%1) ) ) )
...
def go(cntlPath, expPath):
    ...
    for i,f in enumerate(cntList):
        ...
        cntMw = eMw.shape[1]
        cntLw = eLw.shape[1]
        idxOutOfControl = np.where( np.isin(eTimes, cTimes, invert=True) ) 
        with h5py.File('idxOutOfControl.h5','w') as f:
            dset = f.create_dataset('idxOutOfControl',data = idxOutOfControl)
        # ... much like how this investigation into polarization correction has gone.
        for i in range(len(wnLw)):
            if wnLw[i] in wnAssim:
                ...
                plt.hist(eLw[i,idxOutOfControl],bins='auto')
                plt.ylabel('Number of Observations [count]')
                plt.xlabel('Brightness Temperature [K]')
        ...
        lwAveTmp = eLw.mean(axis=1) - cLw.mean(axis=1)
        mwAveTmp = eMw.mean(axis=1) - cMw.mean(axis=1)
        swAveTmp = eSw.mean(axis=1) - cSw.mean(axis=1)
